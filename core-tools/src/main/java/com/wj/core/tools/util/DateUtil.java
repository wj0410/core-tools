...
public class DateUtil {
    ...
    /**
     * @Description 获取当前对应数据库datetime类型的时间
     * @Date 2020/4/10/010 14:59
     **/
    ...
    public static String getLastDayOfMonth(String yearMonth) {
        ...
        String ye = yearMonth.substring(0, 4);
        String mo = yearMonth.substring(5, 7);
     ...
     * @return 格式:MM_dd
     */
    public static List<String> getBeteenDateList(String begin, String end) {
        List<String> hashList = new ArrayList<>();
        String btime = begin.substring(0, 10);//yyyy-MM-dd
        String etime = end.substring(0, 10);

        Date bDate = cn.hutool.core.date.DateUtil.parse(btime, DatePattern.NORM_DATE_PATTERN);//yyyy-MM-dd
        Date eDate = cn.hutool.core.date.DateUtil.parse(etime, DatePattern.NORM_DATE_PATTERN);
        List<DateTime> dateList = cn.hutool.core.date.DateUtil.rangeToList(bDate, eDate, DateField.DAY_OF_YEAR);//创建日期范围生成器
        String hash = null;
        for (DateTime dt : dateList) {
            hash = dt.toString().substring(0, 10) + "(" + getDayOfWeek(dt.toString().substring(0, 10)) + ")";
            hashList.add(hash);
        }
        return hashList;
    }
     ...
     * @return
     */
    public static List<String> getRangeYears(List<String> years, String startYear, String endYear) {
        years.add(startYear);
        Calendar cal = Calendar.getInstance();
        cal.setTime(parseDate(startYear + "-01-01"));
        cal.add(Calendar.YEAR, 1);//增加一年
        Date nextDate = cal.getTime();
        if (nextDate.compareTo(parseDate(endYear + "-01-01")) <= 0) {
            return getRangeYears(years, formatDate(nextDate, "yyyy"), endYear);
        }
        return years;
    }
//    public static void main(String[] args) {
//        List<String> years = getRangeYears(new ArrayList<>(), "2011", "2021");
//        System.out.println(years.size());
//        System.out.println(getBeteenDateList("2021-01-01 12:12:12","2021-01-31 12:12:12"));
//    }
}
