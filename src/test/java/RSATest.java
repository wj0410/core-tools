//import org.junit.Test;
//
//import static com.wj.core.tools.util.RSAUtils.decryptByPbKey;
//import static com.wj.core.tools.util.RSAUtils.encryptByPvKey;
//
//public class RSATest {
//    @Test
//    public static void main(String[] args) throws Exception {
////        genkeyPair();
//        String publicKey = "";
//        String privateKey = "";
//        String content = "{\"content\":\"测试加密数据JSON\"}";
////        // 使用公钥加密
////        String ctext = encryptByPbKey(content, publicKey);
////        System.out.println(ctext);
////        // 使用私钥解密
////        System.out.println(decryptByPvKey(ctext,privateKey));
//
//        // （数字签名） 使用私钥加密，公钥解密
//        String pvtext = encryptByPvKey(content, privateKey);
//        System.out.println(pvtext);
//        System.out.println(decryptByPbKey(pvtext, publicKey));
//    }
//}
